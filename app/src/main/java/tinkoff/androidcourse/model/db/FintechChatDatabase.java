...
public final class FintechChatDatabase {
    ...
    public static class UpdateLastMessageText extends BaseMigration {
        ...
        public void migrate(DatabaseWrapper database) {
            // run some code here
            Where<MessageItem> maxDateForDialogId = SQLite.select(Method.max(MessageItem_Table.date))
                                                          .from(MessageItem.class)
                                                          .where(MessageItem_Table.dialogItem_id.eq(DialogItem_Table.id));
            Where<MessageItem> textOfLastMessage = SQLite.select(MessageItem_Table.text)
                                                         .from(MessageItem.class)
                                                         .where(MessageItem_Table.dialogItem_id.eq(DialogItem_Table.id))
                                                         .and(MessageItem_Table.date.eq(maxDateForDialogId));
